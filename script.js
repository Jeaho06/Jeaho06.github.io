...
function logMove(message) {
  ...
  messageElem.innerHTML = message; // 기호(!!, ??)를 HTML로 렌더링
...
function aiMove() {
    ...
    if (winMove) {
        ...
        if(checkWin(board, -1)) logReason("시스템", "AI가 승리했습니다!");
        isAITurn = false;
        return;
    }
...
function placeBomb() {
    ...
    if (move && board[move.row][move.col] === 0) {
        ...
        logReason("AI", "특별한 돌을 하나 설치했습니다. 다음 제 차례에 터집니다...");
        isAITurn = false;
        return true;
    }
    return false;
...
function performNormalMove(move = null) {
    ...
    if (move && board[move.row][move.col] === 0) {
        ...
        if(fallbackMove && board[fallbackMove.row][fallbackMove.col] === 0) {
             ...
             logMove(`AI: ${aiCoord}`);
             logReason("AI", "둘 곳이 마땅치 않네요.");
        } else {
            logReason("시스템", "더 이상 둘 곳이 없습니다.");
        }
    }
}
...
function performStoneSwap() {
    ...
    if (aiStone) {
        ...
        logReason("AI", `당신의 돌(${userCoord})과 제 돌(${aiCoord})을 바꿨습니다.`);
...
// --- 이하 유틸리티 및 규칙 확인 함수 (호출 부분만 logReason으로 변경) ---
